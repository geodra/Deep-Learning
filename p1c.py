...
def model(x_train, y_train, x_test, y_test,  model_file, path, batch_size = 128, num_epochs = 20, LR = 0.001,training_mode = True, save_model = True, save_plot =True):
    ...
    ## Variables ##
    W1 = tf.Variable(tf.random_normal([784, hidden_units], stddev=0.01))
    b1 = tf.Variable(tf.random_normal([hidden_units], stddev=0.01))
    layer1 = tf.nn.relu(tf.matmul(x,W1) + b1)

    W2 = tf.Variable(tf.random_normal([hidden_units, hidden_units], stddev=0.01))
    b2 = tf.Variable(tf.random_normal([hidden_units], stddev=0.01))
    layer2 = tf.nn.relu(tf.matmul(layer1, W2) + b2)

    W3 = tf.Variable(tf.random_normal([hidden_units, 10], stddev=0.01))
...
def main(model_file, path, train=True, n_train=1000, n_test=200, save_model=True, save_plot = True):
    ...
    with timer("Run model"):
        model(x_train, y_train, x_test, y_test, model_file, path, batch_size=128, num_epochs=20, LR=0.0001, training_mode=train, save_model = save_model, save_plot = save_plot)
